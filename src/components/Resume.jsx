  ...
  return (
    <section className="px-6 py-20 md:py-28 min-h-screen bg-[rgba(26,29,39,0.3)]">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {/* How I work */}
        <div className="mb-16 p-6 rounded-xl bg-[#0f1117] border-l-4 border-l-[#3b82f6] border border-[#2a2d37]">
          <h3 className="text-xl font-semibold text-white mb-4">
            {resume.howIWork.heading}
          </h3>
          <div className="grid sm:grid-cols-2 gap-3">
            {resume.howIWork.items.map((item, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-[rgba(26,29,39,0.5)]">
                <span className="w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0 bg-[#3b82f6]" />
                <p className="text-sm leading-relaxed text-[#e5e7eb]">{item}</p>
              </div>
            ))}
          </div>
        </div>
